...
const TopicWordsScreen = ({ 
  ...
  // const topicKeys = [ ... ]; // Không dùng nữa
  return (
      ...
      <main className={`transition-all duration-300 flex flex-col items-center justify-start w-full overflow-y-auto ${showMenu && !isFullscreen ? 'pl-44' : ''} ${isFullscreen ? 'fixed inset-0 z-50 bg-white' : ''}`}
        ...
        <div className="grid grid-cols-3 lg:grid-cols-4 gap-6 max-w-6xl w-full p-6 sm:p-8">
          {lessons.map((lesson, index) => {
            ...
            if (i18n.language === 'en') lessonTitle = lesson.title_en;
            if (i18n.language === 'ja') lessonTitle = lesson.title_ja;
            return (
              <button
                key={index}
                onClick={() => {
              ...
              >
                ...
                <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gray-800 text-center mb-1 truncate break-words max-w-full text-ellipsis whitespace-normal">{lessonTitle}</h3>
                <p className="text-gray-600 text-center text-xs sm:text-sm">{t('topic.words_count', { count: lesson.words.length })}</p>
              </button>
            );
          })}
        </div>
